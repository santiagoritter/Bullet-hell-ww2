...
func _physics_process(delta):
	...
	if Input.is_physical_key_pressed(KEY_W) and esta_atacando == false:
		direccion_Y -= 1
	if Input.is_physical_key_pressed(KEY_S) and esta_atacando == false:
		direccion_Y += 1
	if Input.is_physical_key_pressed(KEY_A) and esta_atacando == false:
		$AnimatedSprite2D.flip_h = true
		direccion_X -= 1
	if Input.is_physical_key_pressed(KEY_D) and esta_atacando == false:
	...
	elif Input.is_physical_key_pressed(KEY_E) and esta_atacando == false:
		esta_atacando = true
...
func recibir_danio(cantidad):
	vida = vida - cantidad
	$CanvasLayer/ProgressBar.value = vida
	print("me pego una bala, me quedan ", vida, " vidas")
	
	if vida <= 0:
		print("me mori, reiniciando nivel...")
